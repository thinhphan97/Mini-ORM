"""Async low-level CRUD/query implementations used by `AsyncRepository`."""
...
async def insert(repo: Any, obj: Any) -> Any:
    ...
    if repo.d.paramstyle == "named":
        ...
        if repo.meta.auto_pk and repo.d.supports_returning:
            sql += repo.d.returning_clause(repo.meta.auto_pk) + ";"
            row = await repo.db.fetchone(sql, params)
            if row and repo.meta.auto_pk in row:
                setattr(obj, repo.meta.auto_pk, row[repo.meta.auto_pk])
            return obj

        sql += ";"
        cursor = await repo.db.execute(sql, params)
    else:
        ...
        if repo.meta.auto_pk and repo.d.supports_returning:
            sql += repo.d.returning_clause(repo.meta.auto_pk) + ";"
            row = await repo.db.fetchone(sql, values)
            if row and repo.meta.auto_pk in row:
                setattr(obj, repo.meta.auto_pk, row[repo.meta.auto_pk])
            return obj

        sql += ";"
        cursor = await repo.db.execute(sql, values)
...
async def update(repo: Any, obj: Any) -> int:
...
async def delete(repo: Any, obj: Any) -> int:
    ...
    if repo.d.paramstyle == "named":
        ...
        cursor = await repo.db.execute(sql, {"pk": pk_value})
        return cursor.rowcount
    ...
    cursor = await repo.db.execute(sql, [pk_value])
    return cursor.rowcount


async def get(repo: Any, pk_value: Any) -> Any:
...
async def list_rows(
    ...
    *,
    ...
    rows = await repo.db.fetchall(sql + ";", params)
    return [row_to_model(repo.model, row) for row in rows]


async def count_rows(repo: Any, *, where: Any = None) -> int:
    ...
    row = await repo.db.fetchone(sql + ";", where_fragment.params)
    if not row:
        return 0
    return int(row["__count"])


async def exists_rows(repo: Any, *, where: Any = None) -> bool:
...
async def insert_many(repo: Any, objects: Sequence[Any]) -> list[Any]:
    """Insert many objects and return inserted objects."""

    inserted: list[Any] = []
    for obj in objects:
        inserted.append(await repo.insert(obj))
    return inserted


async def update_where(repo: Any, values: Mapping[str, Any], *, where: Any) -> int:
...
async def delete_where(repo: Any, *, where: Any) -> int:
    ...
    cursor = await repo.db.execute(sql, where_fragment.params)
    return cursor.rowcount
...
async def get_or_create(
    ...
    *,
    ...
    found = await repo.list(where=conditions, limit=1)
    if found:
